...
class Lgsvg {
    ...
    drawElectricA(x, alist) {
        ...
        for (var i = 0; i < length; i++) {
            ...
            (function (i) {
                if(alist[i].dbbs == 'db226'){
                    var imgid = dsvg.drawImg("img/db.svg", 115 + (t * 50) + x - 20, 75 + Math.floor(i / 20) * 85 + 75, alist[i].dbbs, () => {
                        layer.open({
                            title:'db226',
                            type: 2,
                            content: '/ltvs_management/modules/mdsysset/pic5.html',
    ...
    drawElectricB(x, blist, lev) {
        ...
        for (var i = 0; i < length; i++) {
            ...
            (function (i) {
                if(blist[i].dbbs == 'db127'){
                    var imgid = dsvg.drawImg("img/db.svg", 115 + (t * 50) + x - 20, 75 + Math.floor(i / 20) * 85 + 75 + lev * 85, blist[i].dbbs, () => {
                        layer.open({
                            title:'db127',
                            type: 2,
                            content: '/ltvs_management/modules/mdsysset/pic4.html',
                            area: [OPEN_WIDTH*0.5 + "px", OPEN_HEIGHT*0.6 + "px"],
                            maxmin: false
                        });
                    },'click');
